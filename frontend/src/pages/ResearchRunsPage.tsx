...
export function ResearchRunsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    await action.run(async () => {
      const run = await research.createRun({ query: query.trim(), depth: 'standard', source: 'manual' })
  ...
  return (
    <section className="workbench-page research-page">
      ...
      <form className="hero-input-panel" onSubmit={submit}>
        ...
        <div className="hero-input-footer"><span className="muted small">建议输入具体对象、判断目标和期望产出。</span><button className="button" disabled={action.busy || !query.trim()}>{action.busy ? '创建中…' : '创建研究'}</button></div>
      </form>
      <ActionNotice message={action.message} error={action.failed} />
      {error ? <ErrorState message={error} /> : loading ? <LoadingState title="正在加载研究记录" /> : !runs.length ? <EmptyState title="暂无研究记录" description="你可以从信息流发起研究，也可以在这里直接创建。" /> : (
        <div className="record-list" ref={listMotion}>{runs.map((run) => <Link className="record-card" to={`/research/${run.id}`} key={run.id} data-entry={run.id}><div className="record-main"><div className="row"><StatusPill value={run.status} /><span className="muted small">{run.completed_at || '等待完成'}</span></div><h2>{run.query || '未命名研究'}</h2><p>成果 {run.artifact_id ? '已生成' : '未生成'} · 技能草稿 {run.skill_draft_id ? '已创建' : '暂无'}</p></div><span className="record-arrow">→</span></Link>)}</div>
      )}
    </section>
  )
}
