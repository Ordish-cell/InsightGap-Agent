...
export function FeedPage() {
  ...
  return (
    <section className="workbench-page feed-page">
      <PageHeader title="信息雷达" description="重点查看价值、证据、推荐原因和下一步行动。所有真实内容均来自后端 FeedCard。" actions={<button className="button" onClick={async () => { await feed.refresh(); await load() }}>刷新雷达</button>} />
      <div className="metric-row">
        <div className="metric-card"><strong>{stats?.cards_count ?? cards.length}</strong><span>信息卡片</span></div>
