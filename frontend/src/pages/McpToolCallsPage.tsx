...
export function McpToolCallsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const nextCalls = await mcp.listToolCalls()
      setTools(nextTools)
      setCalls(nextCalls)
      setSelected(previous => nextTools.find(tool => tool.name === previous?.name) || nextTools[0] || null)
    } catch (exc) {
  ...
  return (
    <section className="workbench-page mcp-page">
      <PageHeader title="工具审计" description="查看可用工具、权限与执行记录。" actions={<button className="button secondary" onClick={load} disabled={loading}>{loading ? '刷新中…' : '刷新'}</button>} />
      {error ? <ErrorState message={error} /> : loading ? <LoadingState title="正在加载工具审计" /> : (
        <div className="mcp-layout">
          ...
          <aside className="selected-tool-panel"><span className="eyebrow">工具详情</span><h2>{selected?.name || '未选择工具'}</h2><p>{selected?.description || '选择左侧工具查看说明。'}</p>{selected?.safety_level ? <StatusPill value={selected.safety_level} /> : null}<details><summary>技术详情</summary><JsonBlock value={selected} /></details></aside>
          <section className="tool-call-panel"><div className="section-title"><strong>调用记录</strong><span>{calls.length} 条</span></div>{!calls.length ? <EmptyState title="暂无工具调用" /> : <div className="call-list">{calls.map((call) => <article className="call-card" key={call.id}><div className="call-card-head"><StatusPill value={call.status} /><StatusPill value={call.safety_level} /></div><strong>{call.tool_name}</strong><p>{call.error ? `错误：${call.error}` : '调用已记录，输入输出已收起。'}</p><details><summary>技术详情</summary><JsonBlock value={{ user_id: call.user_id, input: call.input, output: call.output }} /></details></article>)}</div>}</section>
        </div>
      )}
    </section>
  )
