...
const output = join(tmpdir(), 'insight-native-progress'); await mkdir(output, { recursive: true })
try {
  ...
  emit(1, 'agent_text_delta', { message_id: 'assistant-1', text_id: 'native-1', text: '听起来你很累，' })
  // Must paint before the rest of the text or any completion event exists.
  await page.getByText('听起来你很累，', { exact: true }).waitFor()
  emit(1, 'agent_text_delta', { message_id: 'assistant-1', text_id: 'native-1', text: '先歇一会儿吧。' })
  await page.getByText('听起来你很累，先歇一会儿吧。', { exact: true }).waitFor()
  emit(1, 'agent_text_completed', { message_id: 'assistant-1', text_id: 'native-1', role: 'final', text: '听起来你很累，先歇一会儿吧。' })
  emit(1, 'answer_delta', { message_id: 'assistant-1', text_id: 'native-1', text: '听起来你很累，先歇一会儿吧。' })
  const finish = id => {
  ...
  emit(2, 'interaction_mode', { mode: 'chat', interaction_version: 2 })
  emit(2, 'agent_text_started', { message_id: 'assistant-2', text_id: 'native-2' })
  emit(2, 'agent_text_delta', { message_id: 'assistant-2', text_id: 'native-2', text: '我先检查调用记录。' })
  await page.locator('.answer-content').filter({ hasText: '我先检查调用记录。' }).waitFor()
  await page.evaluate(() => { window.progressElement = document.querySelector('[data-work-block="2:native-2"]') })
  emit(2, 'agent_text_completed', { message_id: 'assistant-2', text_id: 'native-2', role: 'progress', text: '我先检查调用记录。' })
  await page.locator('[data-text-role=progress]').filter({ hasText: '我先检查调用记录。' }).waitFor()
  assert.equal(await page.evaluate(() => window.progressElement === document.querySelector('[data-work-block="2:native-2"]')), true)
  assert.equal(await page.getByText('我先检查调用记录。', { exact: true }).count(), 1)
  emit(2, 'tool_call_started', { tool_call_id: 'read-1', tool_name: 'document.read' })
  await page.locator('.work-tool-row.running').filter({ hasText: 'document.read' }).waitFor()
  emit(2, 'node_started', { step_id: 'inspect', status: 'running', display_name: '读取文档' })
  emit(2, 'tool_call_completed', { tool_call_id: 'read-1', tool_name: 'document.read' })
  emit(2, 'node_completed', { step_id: 'inspect', status: 'completed', display_name: '读取文档' })
  emit(2, 'agent_text_started', { text_id: 'final-2', message_id: 'assistant-2' })
  const samples = []
  let streamed = ''
  for (let i = 0; i < 30; i++) {
    const started = Date.now(), delta = i === 0 ? '根据读取的记录，' : `${i} `
    streamed += delta
    emit(2, 'agent_text_delta', { message_id: 'assistant-2', text_id: 'final-2', text: delta })
    await page.locator('[data-work-block="2:final-2"]').filter({ hasText: streamed.trim() }).waitFor()
    await page.evaluate(() => new Promise(r => requestAnimationFrame(r)))
    samples.push(Date.now() - started)
  }
  emit(2, 'agent_text_completed', { message_id: 'assistant-2', text_id: 'final-2', role: 'final', text: '主要等待来自意图识别。' })
  assert.equal(await page.locator('.live-progress-details').last().getAttribute('open'), null)
