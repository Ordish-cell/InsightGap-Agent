...
try {
  ...
  await page.getByRole('button', { name: '梳理一个研究想法' }).click()
  assert.equal(await page.locator('textarea').inputValue(), '梳理一个研究想法')
  ...
  assert.equal(await page.locator('.floating-feed-card').count(), 3)
  assert.ok(await page.locator('.floating-feed-card').first().evaluate(el => el.getBoundingClientRect().height <= 250), 'home cards should stay compact')
  await page.locator('.floating-feed-card').first().hover()
  await page.waitForTimeout(220)
  assert.notEqual(await page.locator('.floating-feed-card').first().evaluate(el => getComputedStyle(el).transform), 'none')
  await page.locator('.floating-feed-card').first().getByRole('button', { name: '带入对话' }).click()
  assert.equal(await page.locator('.floating-feed-card').first().getByRole('button', { name: '取消带入' }).getAttribute('aria-pressed'), 'true')
  await page.locator('.floating-feed-card').first().getByRole('button', { name: '取消带入' }).click()
  assert.equal(await page.locator('.selected-feed-context').count(), 0)
  await screenshot('home-expanded')
  await page.getByRole('button', { name: /今日精选/ }).click()
  await page.locator('.primary-navigation').getByRole('link', { name: '信息流', exact: true }).click()
  const navTransition = await page.locator('.nav-active-indicator').evaluate(el => getComputedStyle(el).transitionDuration)
  assert.notEqual(navTransition, '0s', 'navigation indicator should have a transition')
  await screenshot('motion-navigation-midpoint')
  await page.waitForTimeout(240)
  await screenshot('motion-navigation-settled')
  await page.locator('.feed-item-card').first().hover()
  await page.waitForTimeout(240)
  assert.notEqual(await page.locator('.feed-item-card').first().evaluate(el => getComputedStyle(el).transform), 'none')
  ...
  for (const width of [390, 768, 1440, 1920]) {
    ...
    const feedOverflow = await page.evaluate(() => ({ scroll: document.documentElement.scrollWidth, viewport: innerWidth }))
    assert.ok(feedOverflow.scroll <= feedOverflow.viewport + 1, `long Feed overflows at ${width}: ${feedOverflow.scroll}`)
