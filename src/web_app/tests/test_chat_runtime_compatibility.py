...
async def test_compiled_graph_forwards_config_to_real_supervisor_method(monkeypatch):
    seen = []
    async def supervisor(nodes, state, runtime_config=None):
        seen.append(runtime_config)
        return state
    monkeypatch.setattr('src.web_app.agent.runtime.capabilities.execute_capability', supervisor)
    nodes = SupervisorNodes(None, {'payload_option': 'kept'})
    graph = StateGraph(dict)
    graph.add_node('supervisor', controlled_node('deep_research', nodes.deep_research, None))
    ...
    token = execution.set(ChatExecution(1, disabled=True))
    try:
    ...
    assert nodes.payload['payload_option'] == 'kept'
...
def test_summary_and_intent_ignore_reasoning_blocks(monkeypatch):
    ...
    from src.web_app.agent.llm.content import message_text as _message_content
